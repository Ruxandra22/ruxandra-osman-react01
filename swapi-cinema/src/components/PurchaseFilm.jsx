...
class PurchaseFilm extends Component {
  ...
  toggleSeatSelect(seatKey) {
    const purchaseSeats = new Set(this.state.selectedSeats);

    if (purchaseSeats.has(seatKey)) {
      purchaseSeats.delete(seatKey);
    } else {
      purchaseSeats.add(seatKey);
    }

    this.setState({
      selectedSeats: purchaseSeats,
    })
  }
      ...
      for (let j = 1; j <= columns; j++ ) {
        ...
        seats.push(
          <button
            ...
            title={seatKey}
            type="button"
            onClick={() => {this.toggleSeatSelect(seatKey)}}
          />
        );
      }
    }
    ...
    return (
      <section className="row">
        <header className="col-12 d-flex mb-4 justify-content-between align-items-center">
          <h2>
            Purchase tickets for {this.props.film.title}
          </h2>
        </header>
